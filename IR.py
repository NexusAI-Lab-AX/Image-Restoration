...
def create_random_mask(image_shape):
    """
    Generate a random polygon mask from a given image size
    """
    height, width = image_shape[:2]
    mask = np.zeros((height, width), dtype=np.uint8)

    # Number of polygons (random)
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.use_mask:
            ...
            # Create a corrupted image by applying a mask to train_gt
...
# train_input Dataset
input_dataset = ImageDataset("/your/path/to/train_input", "/your/path/to/train_gt", use_mask=False)
# Random Mask Dataset
masked_dataset = ImageDataset("/your/path/to/train_input", "/your/path/to/train_gt", use_mask=True)
...
for epoch in range(120):
    if epoch == 40:  # Change learning rate starting from 40 epochs
    ...
    # Save epoch results as ZIP file
